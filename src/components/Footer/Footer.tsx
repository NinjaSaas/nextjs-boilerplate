...
const Footer: React.FC<FooterProps> = ({ className = "", type = "main" }) => {
          ...
          {menu &&
            ...
            menu.menus.map((item, index) => (
              <li key={index}>
                <a
                  key={index}
                  className="text-primary-800 hover:text-primary-500 dark:text-neutral-300 dark:hover:text-white"
                  href={item.href}
                >
                  {item.label}
                </a>
  ...
  switch (type) {
    case "main":
      return (
        <>
          {PAGES_WITH_FOOTER_NAV.includes(pathname as PathName) && (
            <FooterNav />
          )}
            ...
            <ul className="flex space-x-3">
              <li>
                <Link href="https://morovento.com">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gray-50 transition ease-in-out duration-500 hover:bg-primary-200 hover:text-white hover:shadow-lg">
                    <svg
                      className="w-6 h-6"
                      fill="currentColor"
                      viewBox="0 0 24 24"
