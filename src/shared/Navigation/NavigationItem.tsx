...
import { PathName, Route } from "@/types";
...
export interface NavItemType {
  ...
  activeIcon?: React.FC<{ width: number; hanging: number } | any>;
}
...
const NavigationItem: FC<NavigationItemWithRouterProps> = ({ menuItem }) => {
  ...
  const renderMainItem = (item: NavItemType) => {
    return (
      <Link
        rel="noopener noreferrer"
        className="inline-flex items-center rounded-full px-4 py-2  font-body text-primary-700 hover:bg-primary-50 hover:text-neutral-900 dark:text-neutral-300 dark:hover:bg-neutral-800 dark:hover:text-neutral-200 xl:px-5 xl:text-base"
        href={item.href || "/"}
      >
        {item.name}
      </Link>
  ...
  const renderIconItem = (item: NavItemType) => {
    ...
    return (
      <li
        className={`${
          active ? " border-b-2 border-primary-6000" : ""
      ...
      >
        <Link
          ...
          } inline-flex flex-col items-center text-primary-6000 rounded-lg px-4 py-2    hover:bg-primary-50 hover:text-neutral-900 dark:text-neutral-300 dark:hover:bg-neutral-800 dark:hover:text-neutral-200 xl:px-5 xl:text-base`}
          href={href as Route}
        >
          {item.icon &&
